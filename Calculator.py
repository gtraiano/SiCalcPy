...
class Calculator:
    ...
    def __init__(self):
        ...
        self.valid_expr_pattern = r'^\s*((?<!\d)-?\d+\.?\d*)(?:\s*((?<=\d|\s)[-+*/%]|mod)\s*((?<!\d)-?\d+\.?\d*)\s*)+$|store|recall|clear|help'
    ...
    def extract_operands(self, expr: str) -> None:
        for op in re.findall(self.operands_re, expr):
            self.operands.put(float(op))
    
    def extract_operators(self, expr: str) -> None:
    ...
    def calculate(self, expr: str) -> float:
        ...
        self.extract_operands(stripped_expr)
        self.extract_operators(stripped_expr)
